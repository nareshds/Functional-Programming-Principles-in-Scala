...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def mostRetweeted: Tweet ={
      lazy val leftMaxTweets = left.mostRetweeted
      lazy val rightMaxTweets = right.mostRetweeted

      if(!left.empty && leftMaxTweets.retweets > elem.retweets)
        if(!right.empty && rightMaxTweets.retweets > leftMaxTweets.retweets)
          rightMaxTweets
        else
          leftMaxTweets
      else if(!right.empty && rightMaxTweets.retweets > elem.retweets)
        rightMaxTweets
      else
        elem
    }
